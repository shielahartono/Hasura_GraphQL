...
>>  <br/>
>> -> untuk Request, kita gunakan HTTP Method "GET" untuk mengambil Data.  <br/>
...
>> >> <br/>
>> >> -> Terdapat 2 jenis utama Parameter, yaitu "Query Parameters" & "Path Parameters" <br/>
>> >> <br/>
>> >> #### [1] Query Parameters : <br/>
...
>> >> ```
>> >> 
>> >> [-]  Format penulisan Query Parameter dengan "Multiple Key Value Pair" (Beberapa Key Value Pair) : <br/>
>> >> ```
...
>> >> Biasanya Sort ini dari harga terendah hingga harga tertinggi (ascending). <br/>
>> >> 
>> >> 
>> >> 
>> >> ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ <br/>
