...
>> >> "/123" ini menunjukkan Path ke Data User yang lebih spesifik , yang berarti Kita menginginkan Data User yang memiliki "ID 123"  <br/>  <br/>
>> >> 
>> >> 
>> >> 
>> >> ### {2} Header & Parameter in HTTP Request :   <br/>
>> >>
>> >> ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ <br/>
>>
>> 
